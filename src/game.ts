...
/** All DOM elements the GameController operates on. */
interface GameElements {
  boardEl:       HTMLElement;
  scoreBlueEl:   HTMLElement;
  scoreOrangeEl: HTMLElement;
  currentIconEl: HTMLImageElement;
  playerLabelEl: HTMLElement;
  iconBlueEl:    HTMLImageElement;
  iconOrangeEl:  HTMLImageElement;
}
...
export class GameController {
   ...
   */
  constructor(theme: Theme, player: Player, size: Size) {
    this.theme         = theme;
    this.size          = parseInt(size, 10);
    this.currentPlayer = player as PlayerKey;

    const els          = this.initElements();
    this.boardEl       = els.boardEl;
    this.scoreBlueEl   = els.scoreBlueEl;
    this.scoreOrangeEl = els.scoreOrangeEl;
    this.currentIconEl = els.currentIconEl;
    this.playerLabelEl = els.playerLabelEl;
    this.iconBlueEl    = els.iconBlueEl;
    this.iconOrangeEl  = els.iconOrangeEl;
  ...
  private initElements(): GameElements {
    return {
      boardEl:       document.getElementById('game-board')!,
      scoreBlueEl:   document.getElementById('score-blue')!,
      scoreOrangeEl: document.getElementById('score-orange')!,
      currentIconEl: document.getElementById('current-player-icon')! as HTMLImageElement,
      playerLabelEl: document.getElementById('current-player-label')!,
      iconBlueEl:    document.getElementById('hud-icon-blue')!        as HTMLImageElement,
      iconOrangeEl:  document.getElementById('hud-icon-orange')!      as HTMLImageElement,
    };
  }
  ...
  private checkMatch(): void {
    ...
    if (card1.pairId === card2.pairId) {
      ...
      this.handleMismatch(id1, id2);
    }
  }

  /** Handles a successful card pair match. */
  private handleMatch(id1: number, id2: number): void {
  ...
  private handleMismatch(id1: number, id2: number): void {
    this.locked = true;
    setTimeout(() => {
